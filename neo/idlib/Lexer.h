...
typedef enum
{
	LEXFL_NOERRORS						= IDBIT( 0 ),	// don't print any errors
	LEXFL_NOWARNINGS					= IDBIT( 1 ),	// don't print any warnings
	LEXFL_NOFATALERRORS					= IDBIT( 2 ),	// errors aren't fatal
	LEXFL_NOSTRINGCONCAT				= IDBIT( 3 ),	// multiple strings separated by whitespaces are not concatenated
	LEXFL_NOSTRINGESCAPECHARS			= IDBIT( 4 ),	// no escape characters inside strings
	LEXFL_NODOLLARPRECOMPILE			= IDBIT( 5 ),	// don't use the $ sign for precompilation
	LEXFL_NOBASEINCLUDES				= IDBIT( 6 ),	// don't include files embraced with < >
	LEXFL_ALLOWPATHNAMES				= IDBIT( 7 ),	// allow path seperators in names
	LEXFL_ALLOWNUMBERNAMES				= IDBIT( 8 ),	// allow names to start with a number
	LEXFL_ALLOWIPADDRESSES				= IDBIT( 9 ),	// allow ip addresses to be parsed as numbers
	LEXFL_ALLOWFLOATEXCEPTIONS			= IDBIT( 10 ),	// allow float exceptions like 1.#INF or 1.#IND to be parsed
	LEXFL_ALLOWMULTICHARLITERALS		= IDBIT( 11 ),	// allow multi character literals
	LEXFL_ALLOWBACKSLASHSTRINGCONCAT	= IDBIT( 12 ),	// allow multiple strings separated by '\' to be concatenated
	LEXFL_ONLYSTRINGS					= IDBIT( 13 )	// parse as whitespace deliminated strings (quoted strings keep quotes)
} lexerFlags_t;
