...
// sound shader flags
static const int	SSF_PRIVATE_SOUND =		IDBIT( 0 );	// only plays for the current listenerId
static const int	SSF_ANTI_PRIVATE_SOUND = IDBIT( 1 );	// plays for everyone but the current listenerId
static const int	SSF_NO_OCCLUSION =		IDBIT( 2 );	// don't flow through portals, only use straight line
static const int	SSF_GLOBAL =			IDBIT( 3 );	// play full volume to all speakers and all listeners
static const int	SSF_OMNIDIRECTIONAL =	IDBIT( 4 );	// fall off with distance, but play same volume in all speakers
static const int	SSF_LOOPING =			IDBIT( 5 );	// repeat the sound continuously
static const int	SSF_PLAY_ONCE =			IDBIT( 6 );	// never restart if already playing on any channel of a given emitter
static const int	SSF_UNCLAMPED =			IDBIT( 7 );	// don't clamp calculated volumes at 1.0
static const int	SSF_NO_FLICKER =		IDBIT( 8 );	// always return 1.0 for volume queries
static const int	SSF_NO_DUPS =			IDBIT( 9 );	// try not to play the same sound twice in a row
static const int	SSF_VO =				IDBIT( 10 ); // VO - direct a portion of the sound through the center channel (set automatically on shaders that contain files that start with "sound/vo/")
static const int	SSF_MUSIC =				IDBIT( 11 ); // Music - Muted when the player is playing his own music
