...
//    int				x; //Not needed 
	...
	// [FG] initialize composite background to palette index 0 (usually black)
  	memset(block, 0, ::g->s_texturecompositesize[texnum]);
    patch = texture->patches;
		
	for (i = texture->patchcount, patch = texture->patches;
		--i >=0;
		 patch++)
	{
		...
		for (int x = x1; x < x2; x++)
		{
		   ...
		   // if (collump[x] >= 0)
			...
			{      // Column has multiple patches?
				...
				patchcol = (postColumn_t *)((byte *)realpatch + LONG(cofs[x]));
	...
	for (i = 0; i < texture->width; i++)
		//if (collump[i] == -1)                 // process only multipatched columns
		{
			...
			for (;;)  // reconstruct the column by scanning transparency marks
			{
				...
				col = (postColumn_t *)((byte *)col + len + 4); // next post
				}
			}
      ...
      for (i = texture->patchcount, patch = texture->patches; --i >= 0;)
	{
	  ...
	  for (x = x1 ; x<x2 ; x++)
	    //if (count[x].patches > 1)        // Only multipatched columns
	      {
		const postColumn_t *col =
		  (postColumn_t*)((byte*) realpatch+LONG(cofs[x]));
  ...
  int		col )
{
    ...
    // if (lump > 0)
	// return (byte *)img2lmp(W_CacheLumpNum(lump, PU_CACHE_SHARED), lump)+ ofs;
...
// pixel
void R_GenerateSkyHead(int lump, bool fire = false, fireSky_t fireSky = {}) {
	postColumn_t * column;
	byte*			source;
	byte src = 0;
	int pos = 0;
...
void R_SetupFireSky(int tex, fireSky_t fireSky) {
	...
	for(int i = 0; i < FIRESKY_WIDTH; i++) {
		::g->fireBuffer[(FIRESKY_HEIGHT-1)*FIRESKY_WIDTH + i] = 31;
	}
	...
	R_GenerateSkyHead(lump, true, fireSky); //Regenerate the fake sky teture to just black
}
...
void R_GenerateFireSky(int tex, fireSky_t fire) {
	...
	for (int x = 0; x < FIRESKY_WIDTH; x++) {
		if (::g->menuactive) {
			break;
		}
		for (int y = 1; y < FIRESKY_HEIGHT; y++) {
			if(::g->menuactive) {
				break;
			}
			R_StartTheFire(y * FIRESKY_WIDTH + x);
		}
	}
...
// R_GetFireSkyColumn
//
// Draw and return the requested column from the firesky simulation
byte* R_GetFireSkyColumn(int tex, int col, fireSky_t fire) {
	col &= ::g->s_texturewidthmask[tex];

	//Render the wanted column in the final buffer using the palette from fire's definition
	for (int y = 0; y < FIRESKY_HEIGHT; y++) {
		::g->finalFireBuffer[y] = fire.palette[::g->fireBuffer[y * FIRESKY_WIDTH + col]];
	}
...
void R_InitTextures (void)
{
	...
	if (::g->s_numtextures == 0)
	{
		...
		::g->s_texturecolumnofs = (unsigned int**)DoomLib::Z_Malloc (::g->s_numtextures*sizeof(unsigned int*), PU_STATIC_SHARED, 0);
		...
		temp3 = 8 + (temp2-temp1+255)/128 + (::g->s_numtextures+255)/128;
		I_Printf("[");
		for (i = 0; i < temp3; i++)
			I_Printf(" ");
		I_Printf("]");
		...
		for (i=0 ; i < ::g->s_numtextures ; i++, directory++)
		{
			...
			for (j=0 ; j<texture->patchcount ; j++, mpatch++, patch++)
			{
				...
				if (patch->patch == -1)
				{
					I_Printf("R_InitTextures: Missing patch in texture %s",
					texture->name);
					patch->patch = W_CheckNumForName("TNT1A0");
				}
			}		
			::g->s_texturecolumnlump[i] = (short*)DoomLib::Z_Malloc (texture->width*2, PU_STATIC_SHARED,0);
			::g->s_texturecolumnofs[i] = (unsigned int*)DoomLib::Z_Malloc (texture->width*sizeof(unsigned int), PU_STATIC_SHARED,0);//GK:Use sizeof(unsigned) for better stability
